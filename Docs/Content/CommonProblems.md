---
uid: bda126a1-e534-4172-81dc-35a32d91e4cc
alt-uid: CommonProblems
title: Common Problems
keywords: "spell check, common problems"
---

This topic covers some common problems that you may encounter while using the solution/project spell
...
If a file is currently open in an text editor, the spell checker will make an attempt to get the
...
content will not be detected until the file is saved and the spell checking process is ran again.


As mentioned, when an issue is corrected, the tool window will open the file in a text editor
...
view of the file.  In such cases, making changes to the text editor will update the related custom editor.


In cases where the content on disk contains issues but differs from the unsaved content, you may be
...
## Editing Spell Checked Files

When an issue in the solution/project spell checking tool window is corrected, it will
...
spell checking tool window back in synch, rerun the spell checking process.



## Differences in Issues Reported

The solution/project spell checker uses a different method of classifying text to find items that
...
between what is reported by it and the spell check as you type and active document spell checking tool window.


A common difference is doubled word detection.  The interactive spell checkers cannot detect
...
which is not shown.  You can use the **Go To Issue** option to verify that this is the case and handle the
issue accordingly.


The solution/project text parser may report issues incorrectly under certain conditions.  For
...
problem has been reported as noted above.  It may be possible to adjust the classifiers to handle some cases.



## Reducing Misspelling Reports for Non-Words

In many cases, a large number of non-words will be reported as misspellings.  Examples include
...
the [](@6216eedb-6434-4cad-be06-576814e0b735) topic for more information and some examples.



## Words Split Across String Literals
<!-- Ignore spelling: separ ated -->

A common occurrence in designer code, especially in Windows Forms projects, is words that get split
...
layout of the string in the designer file.



## File Content Incorrectly Treated as Plain Text

As mentioned, the solution/project spell checking tool window uses its own set of classifiers to
...
If you see a file in which a large number of issues are reported that appear to be code or other
...
[](@0ff35371-69b5-48dd-a062-037abe2469de) topic to define a new classifier for any unknown file
types.  If you do so, feel free to submit these new classifications by opening a new issue so that they can be
included in a future release.



## HTML File Content Issues

The current HTML classifier should handle most HTML content without issue.  In addition, it
...
my area of expertise so help may be required in resolving some issues.
