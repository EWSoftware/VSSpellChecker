...
// Updated : 06/02/2026
// Note    : Copyright 2023-2026, Eric Woodruff, All rights reserved
//
...
public class CSharpSpellCheckCodeAnalyzer : DiagnosticAnalyzer
{
    ...
    /// Static constructor
    /// </summary>
    /// <remarks>Dependencies are embedded as resources inside this assembly so that the analyzer is fully
    /// self-contained.  Roslyn may copy the analyzer DLL to a cache folder without its dependencies.
    /// The <see cref="AppDomain.AssemblyResolve"/> handler below loads the required assemblies directly
    /// from the embedded resource stream so they are always found regardless of the load location.</remarks>
    static CSharpSpellCheckCodeAnalyzer()
    {
        AppDomain.CurrentDomain.AssemblyResolve += (s, e) =>
        {
            ...
            if(!String.IsNullOrWhiteSpace(e.Name) && !referenceAssemblies.TryGetValue(e.Name, out refAsm))
            {
                ...
                if(pos != -1)
                {
                    ...
                    string shortName = e.Name.Substring(0, pos) + ".dll";
                    ...
                    if(resourceName != null)
                    {
                        ...
                        if(stream != null)
                        {
                            ...
                            if(bytesRead == bytes.Length)
                                refAsm = Assembly.Load(bytes);

                            if(refAsm != null && refAsm.FullName != e.Name)
                                refAsm = null;
                        }
                    }
                }
